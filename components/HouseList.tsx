...
export const HouseList: React.FC<Props> = ({ data }) => {
  ...
  return (
    <div className="bg-reiki-card/80 backdrop-blur border border-slate-800 rounded-xl p-6 h-full print:border-gray-300 print:shadow-none print:bg-white">
      <h3 className="text-xl font-bold text-reiki-magenta mb-6 border-b border-slate-800 pb-2 uppercase tracking-widest print:text-black print:border-gray-300 flex items-center gap-2">
        <span className="w-2 h-2 rounded-full bg-reiki-magenta"></span>
      ...
      <div className="space-y-3 pr-0">
        {data.houses.map((house) => {
          ...
          return (
            <div key={house.houseNumber} className={`flex flex-col py-3 px-4 rounded-xl border transition-all ${isAscendant ? 'bg-gradient-to-r from-reiki-magenta/10 to-transparent border-reiki-magenta/30 shadow-[0_0_10px_rgba(255,0,255,0.05)]' : 'bg-slate-800/40 border-slate-800 hover:bg-slate-800/60 hover:border-slate-700'} print:bg-white print:border-gray-200`}>
              
              {/* Header: Number, Sign, ASC badge, Ruler Name */}
              <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-4">
                     <div className={`w-8 h-8 flex items-center justify-center rounded-full text-sm font-bold shadow-lg ${isAscendant ? 'bg-reiki-magenta text-white' : 'bg-slate-700 text-slate-300'} print:border print:border-gray-400 print:bg-gray-100 print:text-black`}>
                        {house.houseNumber}
                     </div>
                     <div className="flex items-center gap-3">
                        <div className="flex items-center gap-2 text-lg">
                             <span className={`${isAscendant ? 'text-reiki-magenta' : 'text-slate-400'} print:text-black`}>{sign.symbol}</span>
                             <span className={`font-serif tracking-wide ${isAscendant ? 'text-white' : 'text-slate-200'} print:text-black`}>{sign.name.toUpperCase()}</span>
                        </div>
                        {isAscendant && <span className="text-[10px] bg-reiki-cyan text-black px-1.5 py-0.5 rounded font-bold tracking-wider">ASC</span>}
                     </div>
                  </div>

                  <div className="text-right">
                     <div className="text-[10px] text-slate-500 uppercase tracking-widest mb-0.5">Regente</div>
                     <div className="flex flex-col items-end">
                         {house.rulers.map((r, i) => (
                             <span key={i} className={`text-sm font-medium ${isAscendant ? 'text-reiki-cyan' : 'text-reiki-cyan/80'} print:text-black`}>
                                 {r.name}
                             </span>
                         ))}
                     </div>
              ...
              {/* Description */}
              <div className="text-xs text-slate-400 pl-12 border-t border-dashed border-slate-700/50 pt-2 space-y-1 print:text-gray-600 print:border-gray-300">
